...
- **画布自适应容器宽度**：`ResizeObserver` 监听视口按比例缩放（`fitGraph` 默认开启，≤100%），消除对象列横向裁切；工具栏可切换「原始大小」并实时显示缩放百分比。
- **连线三态区分**：默认隐藏无调用的静态拓扑边，「显示静态关系」开关展示虚线；有调用边为实线并带调用次数/均耗时标签；回放路径红色高亮。点击节点聚焦——相连节点与边高亮、其余 muted，右侧详情卡展示调用次数/平均耗时/失败数（对象节点带「查看对象数据」下钻入口）。
- **执行回放步骤条**：选择 run/活动后自动播放，步骤条（当前步红实心、已走过淡红、失败步红字小标）可直接点击定位跳步；失败步展示红色错误条（`role="status"` 可达性标注）。
...
- **agentscope jsonb UPDATE 走 hstore（自毁 PG 落库的会话/文件更新）**：`PgAgentStateStore.upsert` 与 `PgBaseStore.put`/`putIfVersion` 用 `UpdateWrapper.set(jsonb列, Map)` 更新已存在状态/workspace 文件时，`UpdateWrapper.set` 不应用实体字段的 `JacksonTypeHandler`，MyBatis `ObjectTypeHandler.setMap` 按 PostgreSQL **hstore** 绑定参数——无 hstore 扩展抛 `No hstore extension installed`、已装扩展抛 `column is of type jsonb but expression is of type hstore`，导致同会话二次写入（`agent_scope_state`/`agent_scope_file` 的 update-Inline）运行期报 `BadSqlGrammarException`、run 失败。修复：jsonb 列的 UPDATE 参数序列化为 JSON 字符串（agentscope 全局 `JsonUtils.getJsonCodec().toJson(...)`），PG 对 jsonb 列自动隐式 cast 字符串（真库实测 `'{"a":1}'` → `{"a":1}`）；实体 INSERT 路径本就经 typeHandler 正常，不受影响。涉及 `PgAgentStateStore.upsert` 的 `content` 与 `PgBaseStore` 三处 `value`（put 主路径、put 的 `DuplicateKey` 并发兜底、putIfVersion 的 CAS）。已按真实 run 验证：同 session 二次 execute 触发 UPDATE，`agent_scope_state` 行数不增、`content` 类型为 jsonb object、全程无 hstore 报错。
- **agentscope 匿名槽 `userId=null` load 抛 IAE（`Failed to load AgentState for slot`）**：agentscope 内部把 `userId=null` 视为合法匿名槽（`slotKey` 用 `__anon__`），`ReActAgent.loadOrCreateAgentStateForSlot` 等路径以 `null` 调 `AgentStateStore.get`/`getList`/`exists`；`PgAgentStateStore.parseTenantId(null)` 严格 fail-loud 抛 `IllegalArgumentException`，被调用方 catch 后打 WARN 并回退 fresh state（伪装掉已有会话记忆）。修复：**读写分离**——读路径（`row`→`get`/`getList`、`exists`、`listSessionIds`）对无法解析租户的 userId（null/空白/非 `tenant-{id}` 格式）返回空结果（匿名槽查无租户数据，读空不串租户）；写路径（`save`/`delete`）保持 `tenant()` 严格 fail-loud，杜绝无主/跨租户写入。已按真实 run 验证：执行期间不再出现 `userId 无法解析租户` WARN。
