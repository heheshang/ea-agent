...
public class AgentscopeAgentEngine implements AgentEngine {
    ...
    private static final String SYS_PROMPT = """
            ...
            约束：
            1. 只使用工具返回的数据；触达前必须核对退订与频控；所有面向用户的回复一律使用中文（含总结、澄清、拒绝、计划），内部思考过程（reasoning/thinking）同样使用中文，输出简洁、可执行。
            ...
            5. 创建运营活动（createCampaign）时必须同时附触发规则 trigger_rule（含 event_type，可按需附 window）；用户未指定触发条件时先澄清或给出推荐（如订单事件 order_placed、优惠券领取 coupon_used）并征得确认，不得创建无触发规则的活动。
    ...
    private static final String SYS_PROMPT_VERSION = "v12";
